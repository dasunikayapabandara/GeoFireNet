...
async def predict_risk(features: WildfireFeatures, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if severity:
            driver_str = ", ".join(result["primary_drivers"]) if result["primary_drivers"] else "Algorithmic rules"
...
@app.patch("/alerts/{alert_id}/acknowledge", response_model=schemas.Alert)
...
@app.patch("/alerts/{alert_id}/resolve", response_model=schemas.Alert)
