...
## 0. The Core Thesis: Reactive vs. Predictive

**Key Narrative**: Shift the defense from "How accurate is the model?" to "How does this change the wildfire response paradigm?".

* **Reactive Model**: Traditional systems rely on **Satellite Sensors** detecting an **Active Fire**. This triggers an **Alert** for **Emergency Response** (Damage Control).
* **Predictive Model (GeoFireNet)**: We use **Pre-Ignition Data** (Climate + Vegetation) to trigger **ML Predictions**. The goal is **Prevention & Early Warning**, allowing intervention *before* the disaster starts.
...
* **Defense**: "I would prioritize integrating live vegetation moisture indices (NDVI) alongside climate data. This would further strengthen our **Predictive Paradigm**: moving away from *Reactive* detection (detecting a fire that has already started) toward a *Predictive* system that identifies High-Risk environments *before* ignition occurs. This shift from 'Damage Control' to 'Prevention' is the ultimate goal of GeoFireNet."
